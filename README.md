...
For a realistic A100 or TPU Colab run, open `colab_householder_rope_scaling_harness.ipynb`. Its first code cell will `git clone` the repo if needed and install the runtime with `uv`, then the notebook offers five A100-ready profiles: `fast_sanity`, `serious_comparison`, `geometry_signal`, `capacity_limit`, and `long_context_stress`. The default `capacity_limit` profile is the current recommendation for a 40GB A100: it uses a 12-layer, 1536-width, 4096-token configuration with `batch_size=4`, runs for 400 steps, and compares `num_reflectors in {0, 16, 32}` so the experiment pushes VRAM and runtime much harder than the earlier small runs. The notebook now streams the harness output line by line through `subprocess.Popen(...)`, which is more reliable in Colab than relying on inherited subprocess output alone, and it writes JSON, JSONL, CSV, and PNG artifacts into `artifacts/`, including dedicated training-dynamics, component-diagnostics, and RoPE-health plots. If that profile OOMs, the recommended fallback is `geometry_signal`.
