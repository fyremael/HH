...
For a realistic A100 or TPU Colab run, open `colab_householder_rope_scaling_harness.ipynb`. Its first code cell will `git clone` the repo if needed and install the runtime with `uv`, then the notebook offers seven A100-ready profiles: `fast_sanity`, `serious_comparison`, `full_signal`, `geometry_signal`, `capacity_limit`, `capacity_max`, and `long_context_stress`. The default `full_signal` profile is the current recommendation when the goal is to get a real quality signal rather than a slice-driven toy readout: it uses the full Wikitext-103 train and validation splits, a 16-layer 1536-width 2048-token model, 1200 training steps, full validation sweeps, and compares `num_reflectors in {0, 16, 32}`. The `capacity_max` profile is still there when the goal is pure VRAM stress testing. The notebook now automatically backs off the batch size on CUDA OOM all the way down to `1`, raises gradient accumulation to stay near the original effective batch when it has to shrink the microbatch, and runs two post-training causal checks on HH variants: one that re-evaluates the model with HH disabled on the same learned weights, and one that folds HH into `q_proj/k_proj` before re-evaluating. It surfaces a compact loss-throughput-memory tradeoff table after the run, streams the harness output line by line through `subprocess.Popen(...)`, and writes JSON, JSONL, CSV, and PNG artifacts into `artifacts/`, including dedicated training-dynamics, component-diagnostics, RoPE-health, and intervention-impact plots. If the batch-size backoff still fails, the recommended manual fallbacks are `capacity_limit` and then `geometry_signal`.
